...
describe('hior', () => {
  ...
  it('can filter items on properties and free text', () => {
    ...
    expect(filterItems(items, {Level: [{value: 'Proces'}]}, '')).toEqual([items[0]])
    expect(filterItems(items, {Level: [{value: 'xyz'}, {value: 'Proces'}]}, '')).toEqual([items[0]])
    expect(filterItems(items, {Level: [{value: 'Proces'}]}, 'some')).toEqual([items[0]])
    expect(filterItems(items, {Level: [{value: 'Proces'}]}, 'anything else')).toEqual([items[0]])
    ...
    expect(filterItems(items, {Type: [{value: 'Ambitie'}], Level: [{value: 'Proces'}], Theme: [{value: 'A'}]}, 'some text')).toEqual([items[0]])
    expect(filterItems(items, {XYZ: [{value: 'abc'}]}, '')).toEqual([])
  })
})
